...
static enum SbsResult run_build_actions(enum ActionType type, SbsExecutor executor, const struct SbsFile *file, struct SbsEnvSection *env, struct SbsTarget *target, struct SbsPresetSection *preset)
{
...
enum SbsResult sbs_build_run(const struct SbsFile *file, struct SbsBuildArguments args)
{
    ...
    struct SbsPresetSection *preset = NULL;
    if (args.preset)
    {
    ...
    struct SbsEnvSection *env = fl_hashtable_get(file->envs, env_name);
    if (!env) 
