...
SbsVariableInfo* sbs_parse_variable(SbsParser *parser)
{
    ...
    for (size_t i = var_token->value.length - 1; true; )
    {
        if (((const char*) var_token->value.sequence)[i] == '.')
        {
            ...
            variable = sbs_varinfo_new_from_slice(&name, &namespace);
            break;
        }
        else if (i == 0)
        {
            variable = sbs_varinfo_new_from_slice(&var_token->value, NULL);
            break;
        }
        i--;
    }
...
SbsVariableInfo** sbs_parse_variable_array(SbsParser *parser)
{
    ...
    SbsVariableInfo **variables = NULL;

    if (var_count != 0)
    {
        // Parse the variables
        variables = fl_array_new(sizeof(SbsVariableInfo*), var_count);
        size_t index = 0;
