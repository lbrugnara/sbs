...
SbsToolchain* sbs_toolchain_resolve(SbsResolveContext *context, const char *toolchain_name)
{
    ...
    if (toolchain_section->condition && !sbs_expression_eval_bool(context->evalctx, toolchain_section->condition))
        return NULL;
    ...
    for (size_t i=0; i < fl_array_length(toolchain_section->entries); i++)
    {
        ...
        if (toolchain_node->condition && !sbs_expression_eval_bool(context->evalctx, toolchain_node->condition))
            continue;
