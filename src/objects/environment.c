...
{
    SbsEnvSection *env_section = fl_hashtable_get(file->envs, env_name);
    ...
    SbsAction **before_actions = sbs_action_resolve_all(file, env_section->actions.before, env_name);
    if (before_actions)
    {
    ...
    SbsAction **after_actions = sbs_action_resolve_all(file, env_section->actions.after, env_name);
    if (after_actions)
    {
